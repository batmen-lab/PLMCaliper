...
TARGET_FA_PATH=
RUN_BATCH=
DB_KHITS="${DB_KHITS:-15177}"
PARSER_KHITS="${PARSER_KHITS:-15177}"
...
while getopts "q:t:" opt; do
    case $opt in
        ...
        t)
            ...
            ;;
        *)
            echo "Usage: bash $0 -p <fasta_file_name> -t <query_target_fasta_path>"
            exit 1
            ;;
    esac
done
...
if [[ -z "$QUERY_FA_PATH" ]]; then
    ...
    echo "Usage: bash $0 -q <query_fasta_path> [-t <target_fasta_path>]"
    exit 1
fi
...
OUTPUT_DIR="$TEMP_DIR/result_plm_${query_basename}"
mkdir -p "$OUTPUT_DIR"
...
set_target_db_path() {
    ...
    if [ -z "$TARGET_FA_PATH" ]; then
        ...
        if [ ! -f "$PLMSEARCH_TARGET_DB_PATH" ]; then
           ...
           CUDA_VISIBLE_DEVICES=$PLMSEARCH_GPU_DEVICES python "$PLMSEARCH_SRC_DIR/plmsearch/embedding_generate_esm1b.py" \
    ...
    for batch in "${BATCHES[@]}"; do
        {
            ...
            run_tmvec_search "$batch" "$OUTPUT_DIR/$(basename $batch .fa).txt"
        } &
