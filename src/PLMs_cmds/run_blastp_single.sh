...
FA_FILE_NAME=$(basename "$QUERY_FA_PATH")
query_basename="${FA_FILE_NAME%.fa}"
TEMP_DATA_SPLIT_DIR="$TEMP_DIR/${query_basename}_batch"
OUTPUT_DIR="$TEMP_DIR/result_blastp_${query_basename}_blosum62_Q11R1"
...
set_blastp_target_db_path() {
    ...
    if [ ! -d "$BLASTP_TARGET_DB_DIR" ]; then
        ...
        # motify from QUERY_FA_PATH to TARGET_FA_PATH on Aug 27 2025
        # makeblastdb -in $QUERY_FA_PATH -parse_seqids -dbtype prot -out "$BLASTP_TARGET_DB_PATH" \
...
python $BASE_DIR/src/utils/parser.py --data_type blastp --input_path $OUTPUT_DIR --save_path $DATA_DIR --max_hits $PARSER_KHITS --basedb $BASEDB\
