...
# ---------------------------------------------------------------------------
# calibrated tables -> FDR curves -> the four CSVs plot.py reads
# ---------------------------------------------------------------------------
def curve_tag(search_method, query_name, decoy_method, weight_method, target_name):
    return f"{search_method}_{query_name}_{decoy_method}_{weight_method}_{target_name}"


def write_curve_csvs(agg: dict, out_dir: Path, tag: str) -> list:
    """One aggregate CSV plus three per-query point clouds, one per boxplot panel."""
    out_dir.mkdir(parents=True, exist_ok=True)
    n_q = len(agg["q_levels"])

    agg_df = pd.DataFrame({
        "q": agg["q_levels"],
        "mean_real_fdr": agg["mean_real_fdr"],
        "mean_power": agg["mean_power"],
        "mean_accuracy": agg.get("mean_accuracy", [float("nan")] * n_q),
        "valid_count": agg["valid_count"],
    })
    agg_df["total_curves"] = agg["total_curves"]
    agg_df["n_unique_qids"] = agg["n_unique_qids"]
    written = [out_dir / f"fdr_curve_agg_{tag}.csv"]
    agg_df.to_csv(written[0], index=False)

    for key, col, stem in (("all_real_fdps_per_q", "real_fdp", "fdr_boxpoints"),
                           ("all_powers_per_q", "power", "power_boxpoints"),
                           ("all_accuracy_per_q", "accuracy", "accuracy_boxpoints")):
        rows = [{"q": float(q), col: float(v)}
                for q, vals in zip(agg["q_levels"], agg.get(key, [])) for v in vals]
        path = out_dir / f"{stem}_{tag}.csv"
        pd.DataFrame(rows, columns=["q", col]).to_csv(path, index=False)
        written.append(path)
    return written


def run_one(target_noisy: Path, calibrated_decoy: Path, decoy_suffix: str,
            out_dir: Path, tag: str, q_levels, n_jobs: int = 1) -> list:
    if should_stream_pair_table(target_noisy, calibrated_decoy):
        gb = file_size_gb(target_noisy) + file_size_gb(calibrated_decoy)
        print(f"  streaming the pair table ({gb:.1f} GB)", flush=True)
        # both estimators return (curves, combined); the combined half is unused here
        df_curve, _ = estimate_fdr_curves_streaming(
            target_noisy, calibrated_decoy, decoy_suffix, n_jobs=n_jobs)
    else:
        df_merge = make_pair_table(str(target_noisy), str(calibrated_decoy), decoy_suffix)
        print(f"  {len(df_merge):,} paired scores", flush=True)
        df_curve, _ = estimate_fdr_curves_all_queries(df_merge)

    df_summary = summarize_fdr_curves_at_q_levels(df_curve, q_levels=q_levels)
    return write_curve_csvs(aggregate_plot_summary(df_summary), out_dir, tag)
...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        description="Ablation step 2: calibrated tables -> FDR curves -> plotting CSVs.")
    ...
    p.add_argument("--csv-dir", type=Path, default=None,
                   help="default: <out-dir>/plot_data")
    return p.parse_args()
...
def main():
    ...
    from calibrate import DECOY_SUFFIX, combo_tag
